...
```bash
Создадим новую таблицу со строкой меньшего размера, чем в предыдущем примере, 
вставим туда 1 млн. записей со строкой длиной 20 символов:
...
Запускаем анонимный pl/pgsql блок, выполняющий в цикле 10 раз апдейт указанной таблицы, добавляя каждый раз в поле VALUE
номер цикла, в процессе выводится время выполнения каждого цикла:
postgres=# do
$$
declare
    i integer;
